...
const Seating = () => {
  ...
  const handleSubmit = () => {
    ...
    if (storedSelectedTable && storedSelectedTable.id) {
      ...
      navigate(
        `/dinner?partySize=${partySizeParam}&date=${dateParam}&time=${timeParam}&selected_table=${tableSessionId}`
      );
    }
  };
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      myDiagram.nodeTemplateMap.add(
        ...
        $(
          ...
          $(
            ...
            $(
              go.TextBlock,
              { editable: true, font: "bold 8pt Verdana, sans-serif" },
              new go.Binding("text", "name").makeTwoWay(),
              new go.Binding("angle", "angle", (n) => -n)
            )
          ),
      ...
      myDiagram.nodeTemplateMap.add(
        "TableR5", // rectangular with 5 seats
        $(
          go.Node,
          "Spot",
          tableStyle(),
      ...
      let myGuests = new go.Diagram("myGuests", {
